...
| `BATHYS_BROWSER` | `auto` | режим | двухъярусное извлечение: `auto` — HTTP-движок первым, браузер только для JS-страниц; `off` — без браузера; `always` — только браузер (отладка). GitHub-URL (`github.com/*/pull|issues|репо`) обрабатываются отдельным `gh`-API-ярусом независимо от этого режима — это выбор источника (структурный API вместо скрейпа), а не браузерная политика; без `gh` — фолбэк на HTTP-ярус |
