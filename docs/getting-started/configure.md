...
| `BATHYS_DOCS_INDEX` | `$BATHYS_DATA_DIR/docs-index.json` | пользовательский индекс `library_docs`: успешные находки живого поиска дописываются сюда автоматически, следующий вызов той же библиотеки резолвится без сети |
