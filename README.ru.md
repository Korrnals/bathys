...
**0.14.1.** Выпускная история: v0.2 «Качество выдачи» (ретраи, здоровье движков), v0.3 «Паритет с Tavily» (`read_urls`, JSON-режим), v0.4 «Эксплуатация» (robots-этика, метрики, `bathys-doctor`), v0.5 «Identity & Harness» (репозиционирование, промпты, субагент), v0.6 «Native Install» (`bathys install`), v0.7 «Ship & Setup» (двухъярусное извлечение, `bathys setup`, однострочник, uninstall), v0.8 «Engine Orchestration» (оркестрация движков, архком №4), v0.9 «Borrowed Ideas» (find, PDF-ярус, типизация fallback), v0.10–v0.11 «Library Docs» (фазы 1–2), v0.12 «Deep Verdicts» (`source_check`, фаза 3), v0.13 «GitHub Tier & Deep Audit», v0.14 «Backlog Closed» (junk-gate, маркеры определений) — итоги в [CHANGELOG.md](CHANGELOG.md).

Репозиторий: `github.com/Korrnals/bathys`. Пакеты опубликованы: [PyPI `bathys`](https://pypi.org/project/bathys/) (pip install) и [`bathys-mcp` на npm](https://www.npmjs.com/package/bathys-mcp) (npm install -g), однострочник установки — выше. CI и релизы идут через кластерный релизный конвейер (`Korrnals/release-pipeline`: verify-гейты → сборка → SHA256 → SBOM → подписи cosign + GPG → attach к GitHub Release; все 15 релизов прогнаны этим путём — довыясненные версии доложили свои сигнатуры). `.github/workflows/ci.yml` остаётся описанием гейтов; GitHub Actions на аккаунте заблокирован биллингом — его красные прогоны это ожидаемый шум. До 1.0: первый верифицированный прогон Docker-образа (Dockerfile поставляется с пакетом; docker-фаза конвейера требует сначала обновления раннер-образа).
