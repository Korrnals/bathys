...
Репозиторий: `github.com/Korrnals/bathys`. Пакет опубликован: [PyPI `bathys`](https://pypi.org/project/bathys/) (pip install), однострочник установки — выше. CI и релизы идут через кластерный релизный конвейер (`Korrnals/release-pipeline`: verify-гейты → сборка → SHA256 → SBOM → подписи cosign + GPG → attach к GitHub Release; bathys v0.14.1 выпущен этим путём). `.github/workflows/ci.yml` остаётся описанием гейтов; GitHub Actions на аккаунте заблокирован биллингом — его красные прогоны это ожидаемый шум. До 1.0: публикация npm-обёртки `bathys-mcp` и первый верифицированный прогон Docker-образа (Dockerfile поставляется с пакетом, но ещё не прогонялся в контейнерном рантайме).
