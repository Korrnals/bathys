...
Репозиторий: `github.com/Korrnals/bathys`. Пакет опубликован: [PyPI `bathys`](https://pypi.org/project/bathys/) (pip install), однострочник установки — выше. До 1.0: публикация npm-обёртки `bathys-mcp`, первый верифицированный прогон Docker-образа (Dockerfile поставляется с пакетом, но ещё не прогонялся в контейнерном рантайме) и зелёный CI (сейчас заблокирован биллинговой блокировкой GitHub Actions на аккаунте владельца — все прогоны падают с «job was not started»); конфигурация CI (matrix 3.10–3.12 + shellcheck) уже в репозитории.
