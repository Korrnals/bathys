...
- вопрос о библиотеке/фреймворке/API/CLI → `library_docs(library, query=…)`
  — всегда, даже о известной либе и даже если уверен: тренировочные
  данные устаревают, доки — первоисточник; уточняющий вопрос по той же
  либе — снова `library_docs` (кэш сырца: повтор мгновенный и бесплатный);
...
`bathys_fresh_scan`, `bathys_find_docs`: бери как стартовый шаблон плана.
