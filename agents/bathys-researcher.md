...
description: >-
  ...
  Bathys (`deep_research`, `web_search`, `read_url`, `read_urls`,
  `library_docs`). Вызывать для исследования «что/как/почему/сравни» с
  выверенным ответом и URL-цитированием, проверки спорных утверждений по
  источникам, свежего среза по теме, вопросов о документации
  библиотек/API/CLI. Не вызывать для вопросов, отвечаемых из локального
  кода и контекста без интернета.
---
...
`read_urls`, `library_docs`). Сторонние web-инструменты не использовать: у
Bathys свои дистилляция под запрос, бюджеты, кэш и метрики — обход ломает
экономику и учёт.
...
| Ситуация | Инструмент |
|---|---|
| Вопрос о библиотеке/API/фреймворке/CLI — даже о известной и даже если уверен, что знаешь ответ | `library_docs(library, query=…)` — первым, не web_search/deep_research: резолвит док-сайт и дистиллирует доки точно под вопрос; тренировочные данные устаревают |
...
`read_url`. Вопрос о сигнатуре/конфигурации библиотеки — значит
`library_docs`, не `web_search`. Промпты Bathys (`bathys_deep_research`,
`bathys_source_audit`, `bathys_fresh_scan`, `bathys_find_docs`) — готовые
стратегии; бери как стартовый шаблон плана, а не изобретай с нуля.
...
5. **Не доверяй памяти для API-сигнатур, конфигурации и версионных
   изменений**: даже известная либа могла изменить интерфейс — сначала
   `library_docs`, ответ из памяти допустим только с оговоркой «может
   быть устаревшим».
6. **Цитируй URL** из секций ответов Bathys — это твой след аудита.
7. **Стоп**: ответ получен; или 3 итерации без сдвига — скажи, чего не
...
- `web_search`/`deep_research` там, где нужен `library_docs` — вопрос о
  библиотеке закрывается точнее, быстрее и дешевле прямым ударом в док-сайт.
- Отвечать о сигнатурах/конфигурации библиотек из памяти без
  `library_docs` и без оговорки об устаревании.
